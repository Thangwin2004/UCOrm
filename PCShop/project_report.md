...
1. **Singleton Pattern (`DesignPatterns/Singleton/ConfigurationManager.cs`)**
...
2. **Factory Method (`DesignPatterns/FactoryMethod/ProductFactory.cs`)**
...
3. **Abstract Factory (`DesignPatterns/AbstractFactory/...`)**
   - **Mục đích:** Khởi tạo các combo linh kiện (Families of objects) đảm bảo tính tương thích với nhau (Ví dụ: `IntelBuildFactory` chuyên xuất combo Main/CPU Intel, `AmdBuildFactory` xuất combo AMD).
   - **Tác dụng:** Giúp các gói build đề xuất tự động (Pre-built PC) luôn đồng bộ hệ sinh thái.

4. **Builder (`DesignPatterns/Builder/...`)**
   - **Mục đích:** Xây dựng một đối tượng PC phức tạp qua từng bước độc lập (set CPU, set RAM...). Có quản lý qua lớp `PCDirector`.
...
5. **Decorator (`DesignPatterns/Decorator/...`)**
   ...
   - **Tác dụng:** Dùng trong tính năng **Dịch vụ bổ sung** (Base price + Phí bảo hành mở rộng `WarrantyDecorator` + Phí lắp ráp `AssemblyDecorator`).

6. **Repository (`DesignPatterns/Repository/...`)**
   - **Mục đích:** Trừu tượng hoá toàn bộ lớp truy cập cơ sở dữ liệu Entity Framework Core.
   - **Tác dụng:** Logic kinh doanh trong ứng dụng tương tác qua giao diện `IRepository`, không phụ thuộc hay "dính chặt" vào EF Core. Rất tốt cho việc kiểm thử Unit Testing.
...
8. **Strategy (`DesignPatterns/Strategy/...`)**
   ...
   - **Tác dụng:** Hệ thống linh hoạt thay đổi cách tính tiền đơn hàng lúc Checkout như: Giữ nguyên (`RegularPricing`), Trừ % Khuyến mãi (`DiscountPricing`), Giảm sâu + Freeship (`VipPricing`).
...
9. **Observer (`DesignPatterns/Observer/...`)**
   ...
   - **Tác dụng:** Sử dụng trong `OrderService`. Khi Client đặt hàng thành công, hệ thống thông qua `OrderSubject` kích hoạt một loạt các thông báo: Gửi Email cho khách (`EmailNotification`), gửi SMS (`SmsNotification`).

10. **Command (`DesignPatterns/Command/...`)**
    - **Mục đích:** Biến các yêu cầu tương tác thành các Object cụ thể có hàm `Execute()` và `Undo()`.
    - **Tác dụng:** Ứng dụng xuất sắc vào việc "Quản lý giỏ hàng". Khách thêm đồ (`AddToCartCommand`) hoặc lỡ tay gỡ đồ có thể thực hiện hàm `UndoLastAction()` để khôi phục trực tiếp.
...
| ---------- | --------- | ------------------- |
| `PCBuilderController` | Nền tảng luồng nghiệp vụ Interactive Build PC (giống KCCShop). | Gọi AJAX bằng JQuery/Bootstrap Modals để trả về file Razor Partials qua `GetProductsByCategory`. Gửi kết quả dạng Array sang Cart. |
| `CartController` | Giỏ hàng. | Điều khiển `CartService` sử dụng **Command Pattern** (Undo/Redo thao tác giỏ hàng). |
| `OrderController` | Checkout, thanh toán. | Ứng dụng mạnh mẽ **Decorator** (dịch vụ), **Strategy** (tính giá) và **Adapter** (payment proxy). |
| `ProductController` | Duyệt / Render danh mục linh kiện. | Render từ DB qua hệ thống **Repository Pattern**. |
